...
- 写入前自动备份，回滚前也会把当前状态另存一份
- 图形界面 + 命令行两种用法，零第三方依赖（只用 tkinter / urllib / sqlite3）
- 界面跟随 Windows 浅色 / 深色主题（含标题栏），窗口大小与位置自动记住
...
- 点「拉取模型列表」后按钮变「拉取中…」，表格顶部出现进度条，结果填入右侧表格（保持接口返回顺序），同时保存到 `out/models.json` 和 `out/models.txt`。
...
- **排序**：点表头「模型 ID / 名称 / 提供方 / 上下文」循环切换升序 → 降序 → 恢复接口返回顺序，表头箭头指示当前方向。
...
**参数**：上下文 / 最大输出支持 `256000`、`256K`、`1M` 写法。填错会就地红框提示，并在「写入并备份」上方说明原因；当前应用用不到这两项时输入框与「取接口最大值」一起变灰，里面的文字不影响写入。

**按钮**

- **写入并备份**：不可用时上方会说明原因（未勾选模型 / 未选择目标卡 / CC Switch 正在运行 / 参数有误）。
- **预览变更**：只读，先看再写。
- **回滚到备份**：把数据库和 Codex 目录还原到上一次写入前的状态，没有备份或 CC Switch 正在运行时不可用；回滚前会先把当前状态另存一份备份。
...
- 每次写入（含新建卡、删除卡）前都会自动备份到 `~/.cc-switch/backups/`，Codex 目录也会单独备份。同一秒内的多次备份会带上序号，不会互相覆盖。
- 写入后左栏显示最近一次备份文件名，「回滚到备份」按钮变可用，点一下把数据库还原回去。
- 回滚需要先完全退出 CC Switch；回滚前当前状态会另存一份备份，回滚本身也可以再被回滚。
...
| `--force` | 跳过 CC Switch 运行检查（Codex 模式本来就允许运行中写入） |

`apply` 在写入前会检查 CC Switch 是否正在运行，正在运行会直接报错，避免写到一半被覆盖。
...
覆盖四类内容：模型列表解析与排序过滤、token 数值解析、色板对比度检查、界面装配与交互的冒烟测试（构造窗口、注入模型数据、驱动勾选与校验、随即销毁），以及请求、写入、备份、回滚的完整往返测试（本地 HTTP 服务 + 临时数据库）。
...
**拉取失败 / 0 个模型** — 看日志里逐个候选地址的原因。常见：端点缺 `/v1`、网关要求特定 UA、鉴权头不对（换「鉴权」下拉）、返回的是嵌套结构（可直接把可用地址填进「models 地址」）。端点已经是 `/v1/models` 这类模型列表地址时会原样使用，不会再拼一次。
...
| 文件 | 作用 |
| --- | --- |
| `app.py` | 程序入口与 `App` 主窗口（tkinter，零第三方依赖） |
| `ccs_models.py` | 核心逻辑：请求模型列表、读写 CC Switch 数据库/Codex 目录、备份回滚、命令行 |
| `theme.py` | 浅色 / 深色配色、对比度计算、系统主题检测、布局常量 |
| `widgets.py` | 自绘部件：圆角按钮、卡片容器、可滚动纵向容器 |
| `formatting.py` | token 数值解析、上下文格式化、模型列表排序与过滤 |
| `workflows.py` | 预览摘要与写入编排 |
| `tests/` | 单元测试（`python -m unittest discover -s tests`） |
| `pyproject.toml` | ruff 与 mypy 配置（`ruff check .`、`mypy`） |
